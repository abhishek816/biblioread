...
<body>
  ...
  if ($_SERVER["REQUEST_METHOD"] == "POST") {
      ...
      if ($newPassword === $confirmPassword) {
          ...
          $query = "SELECT * FROM pub_signup WHERE email = '$email'";
          $result = $conn->query($query);

          if ($result->num_rows === 1) {
              // User with the provided email exists
              // Update the password for that user
              $updateQuery = "UPDATE pub_signup SET password = '$newPassword' WHERE email = '$email'";
              if ($conn->query($updateQuery) === true) {
                echo '<script>alert("Password updated successfully!!");</script>';
                echo '<script>window.location="publisherlogin.php";</script>';
              } else {
